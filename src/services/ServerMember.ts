...
export const getTopRole = async (serverId: string, userId: string): Promise<CustomResult<ServerRole, CustomError>> => {
  ...
  const member = await prisma.serverMember.findFirst({where: {serverId, userId}, select: {roleIds: true}});
  if (!member) {
    return [null, generateError('Member not found.')];
  }
  member.roleIds.push(server.defaultRoleId);
  const role = await prisma.serverRole.findFirst({where: {id: {in: member.roleIds}}, orderBy: {order: 'desc'}});
  return [role!, null];
};
...
export const updateServerMember = async (serverId: string, userId: string, updatedByUserId: string, update: UpdateServerMember): Promise<CustomResult<UpdateServerMember, CustomError>> => {
  ...
  if (update.roleIds) {
    ...
    const newRoles = await prisma.serverRole.findMany({where: {id: {in: update.roleIds, not: server.defaultRoleId}, serverId}, orderBy: {order: 'desc'}});
    ...
    const oldRoles = await prisma.serverRole.findMany({where: {id: {in: member.roleIds, not: server.defaultRoleId}, serverId}, orderBy: {order: 'desc'}});
