import { removeGoogleDriveAccessTokenCache } from '../cache/UserCache';
...
export const addGoogleDriveConnection = async (userId: string, refreshToken: string) => {
  ...
  const [_, newConnection] = await prisma.$transaction([
    prisma.userConnection.deleteMany({
      where: { provider: ConnectionProviders.GoogleDrive, userId },
    }),
...
export const removeGoogleDriveConnection = async (userId: string) => {
  const user = await prisma.user.findUnique({
    where: {
      id: userId,
    },
  ...
  await removeGoogleDriveAccessTokenCache(userId);
  emitUserConnectionRemoved(userId, connection.id);

  return [true, null] as const;
};

export const addGoogleConnection = async (userId: string, refreshToken: string, googleUserId: string) => {
  const user = await prisma.user.findUnique({
    where: {
      id: userId,
    },
