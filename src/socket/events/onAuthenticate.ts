...
export async function onAuthenticate(socket: Socket, payload: Payload) {
  ...
  if (isFirstConnect && user.status !== UserStatus.OFFLINE) {
    emitUserPresenceUpdate(cacheUser.id, {status: user.status, userId: cacheUser.id}, socket.id);
  }
  ...
  socket.emit(AUTHENTICATED, {
    user: {...cacheUser, email: user.account?.email, orderedServerIds: user.account?.serverOrderIds},
    servers,
    serverMembers,
    serverRoles,
