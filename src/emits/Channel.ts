...
export const emitDMMessageCreated = (
  ...
  const userIds = [
    channel.inbox?.recipientId as string,
    channel.inbox?.createdById as string,
  ];
...
export const emitDMMessageUpdated = (
  channel: BaseChannelCache & DMChannelCache,
  messageId: string,
...
export const emitDMMessageReactionAdded = (
  channel: BaseChannelCache & DMChannelCache,
  reaction: any
) => {
  const io = getIO();
...
export const emitDMMessageReactionRemoved = (
  channel: BaseChannelCache & DMChannelCache,
  reaction: any
) => {
  const io = getIO();
...
export const emitDMMessageDeleted = (
  channel: BaseChannelCache & DMChannelCache,
  ...
  const userIds = [
    channel.inbox?.recipientId as string,
    channel.inbox?.createdById as unknown as string,
  ];

  io.in(userIds).emit(MESSAGE_DELETED, data);
};

export const emitServerChannelCreated = (
  serverId: string,
  channel: Channel
) => {
  const io = getIO();
...
export const emitServerChannelUpdated = (
  serverId: string,
  channelId: string,
  updated: UpdateServerChannelOptions
) => {
  const io = getIO();
...
export const emitServerChannelDeleted = (
  serverId: string,
  channelId: string
) => {
  const io = getIO();
...
export const emitServerTyping = (channelId: string, typingUserId: string) => {
  ...
  io.in(channelId)
    .except(typingUserId)
    .emit(CHANNEL_TYPING, { userId: typingUserId, channelId });
};

export const emitInboxTyping = (
  channelId: string,
  inbox: InboxCache,
  typingUserId: string
) => {
