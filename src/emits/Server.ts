...
export const emitServerClanUpdate = (serverId: string, clan: null | { tag: string; icon: string; serverId: string }) => {
  const io = getIO();
  io.in(serverId).emit(SERVER_CLAN_UPDATE, clan);
};
