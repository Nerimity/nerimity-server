...
export function applicationUpdate(Router: Router) {
  Router.patch(
    ...
    body('redirectUris.*').isString().withMessage('redirectUris must be an array of strings.').isURL({ require_protocol: true, require_tld: false }).withMessage('Each redirectUri must be a valid URL.').optional({}),
