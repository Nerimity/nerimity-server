...
export function channelMessageDelete(Router: Router) {
  Router.delete('/channels/:channelId/messages/:messageId',
    authenticate(),
    channelVerification(),
    memberHasRolePermissionMiddleware(ROLE_PERMISSIONS.MANAGE_CHANNELS, { continueOnError: true }),
    route
  );
}
...
async function route(req: Request, res: Response) {
  ...
  const [isMessageDeleted, error] = await deleteMessage({ channelId: req.channelCache.id, channel: req.channelCache, messageId, serverId: req.channelCache.server?.id, recipientId: req.channelCache.inbox?.recipientId });
