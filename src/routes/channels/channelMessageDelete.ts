...
async function route(req: Request, res: Response) {
  ...
  if (!message)
    return res.status(404).json(generateError('Message not found!'));
  ...
  if (isServerChannel && !isCreatedByMe) {
    if (req.errorMessage)
      return res.status(403).json(generateError(req.errorMessage));
  }
  ...
  if (!isServerChannel && !isCreatedByMe) {
    return res
      ...
      .json(
        generateError(
          'Only the creator of the message can delete this message!'
        )
      );
  }
