...
export function updateServer(Router: Router) {
  Router.post('/moderation/servers/:serverId', 
    authenticate(),
    isModMiddleware,
    body('name')
      .isString().withMessage('Password must be a string!').optional(),
    body('verified')
      .isBoolean().withMessage('Verified must be a boolean!').optional(),
    body('password')
...
async function route (req: Request, res: Response) {
  ...
  const server = await prisma.server.update({
    where: {id: serverId},
    data: update,
    select: {name: true, verified: true, hexColor: true, id: true, createdAt: true, createdBy: {select: {id: true, username: true, tag: true}}, avatar: true}
  });
